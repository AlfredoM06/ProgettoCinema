...
public class OfferteController {
    ...
    public @ResponseBody List<ListaOffertaDTO> filtroOfferte(@RequestParam(name = "filtroOfferta", required = false) String searchOfferta) {
        ...
        for (Offerta offerta : offerte) {
            offerteDTO.add(new ListaOffertaDTO(offerta.getId(), offerta.getNome(), offerta.getGenere(), offerta.getDescrizione(), offerta.getImgBanner(), offerta.getPrezzo(), null, offerta.getDataInizio(), null, 0));
        }
        return offerteDTO;
    }
